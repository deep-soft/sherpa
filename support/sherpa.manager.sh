...
readonly OBJECTS_ARCHIVE_URL="https://raw.githubusercontent.com/OneCDOnly/sherpa/main/$OBJECTS_ARCHIVE_FILE"
...
local -r PACKAGES_ARCHIVE_URL="https://raw.githubusercontent.com/OneCDOnly/sherpa/main/$PACKAGES_ARCHIVE_FILE"
...
readonly THIS_SCRIPT_VER='230218-main'
DebugInfoMajSepr
...
Display "$(FormatAsTitle) $THIS_SCRIPT_VER • a mini-package-manager for QNAP NAS"
...
[[ $qpkgs_skip_processing = true ]] && return
DebugScriptFuncEn
...
[[ $self_debug_to_screen = true ]] && QPKGs.States.List rebuild
DebugScriptFuncEx
}
Action.Proc()
{
...
DebugInfoMajSepr
[[ $self_debug_to_archive = true ]] && ArchiveActiveSessLog
ResetActiveSessLog
EraseThisLine
DisplayLineSpaceIfNoneAlready
return 0
...
if [[ -n $action ]]; then
case $arg in
backedup|failed|help|installable|installed|missing|not-backedup|not-installed|ok|problems|results|skipped|started|stopped|tail|tips)
group=${arg}_
;;
a|abs|alias|aliases)
group=abs_
...
local msg=': '
RefreshForkCounts
[[ $self_debug_to_screen = true ]] && return
...
DebugScriptFuncEn
local package=''
if [[ ${NAS_FIRMWARE_VER//.} -ge 435 ]]; then
ShowAsProc '"signing" all QPKGs'
for package in $(QPKGs.IsInstalled.Array); do
QPKG.Sign "$package"
done
ShowAsDone '"signed" all QPKGs'
fi
DebugScriptFuncEx
}
IPKs.Actions.List()
...
[[ $qpkgs_states_built = true ]] && return
DebugScriptFuncEn
local -i index=0
local package=''
local prev=''
...
echo "$(/bin/date +%s)#${3:?action empty}#${2:?package name empty}#${4:?result empty}#$duration#${5:-}#${1:?package type empty}" >> "$ACTIONS_LOG_PATHFILE"
case $4 in
ok)
DebugAsInfo "${5:-}"
;;
...
QPKG.StoreID()
{
$GETCFG_CMD "${1:?package name null}" store -d sherpa -f /etc/config/qpkg.conf
return 0
}
QPKG.IsBackupExist()
{
...
{
[[ -n $1 ]] || return
[[ -e $GNU_SQLITE_CMD ]] || return
[[ ${NAS_FIRMWARE_VER//.} -ge 435 ]] || return
local qpkg_name=$1
local cert_db_pathfile=/etc/config/nas_sign_qpkg.db
[[ -e $cert_db_pathfile ]] || return
local cert=''
local signature=''
read -r -d '' cert << EOB
-----BEGIN CERTIFICATE-----
...
EOB
$GNU_SQLITE_CMD "$cert_db_pathfile" << EOB
DELETE FROM Certificate WHERE QpkgName = '$qpkg_name';
INSERT INTO Certificate (Type,QpkgName,Cert,DigitalSignature) VALUES ('qpkg','$qpkg_name','$cert','$signature');
EOB
}
